...
export function renderProducts(state) {
  ...
  /**
   * ✅ 3 KHU CHỢ ARCHITECTURE (Quy hoạch lại sản phẩm):
   *
   * 1. HOME PAGE (index.html) → showOnHome: true (featured products)
   *    - Mix của VIP + non-VIP products
   *    - Mục đích: Showcase best-sellers & limited edition
   *    - Filter: where("showOnHome", "==", true)
   *
   * 2. MENU PAGE (products.html) → isVIPOnly: false (general store)
   *    - Chỉ cho khách thường + VIP mua được
   *    - Mục đích: Đại đa số khách hàng
   *    - Filter: where("isVIPOnly", "==", false) && where("available", "==", true)
   *
   * 3. VIP STORE (vip-products.html) → isVIPOnly: true (exclusive)
   *    - Chỉ cho VIP cấp cao (VIP level >= requiredVIP)
   *    - Mục đích: Tạo độ khan hiếm & exclusive feeling
   *    - Filter: where("isVIPOnly", "==", true) && where("available", "==", true)
   *    - PLUS: user VIP level >= product requiredVIP
   */
  ...
  const isVIPStorePage = window.location.pathname.includes("vip-products.html");
  const isMenuPage = window.location.pathname.includes("products.html");

  // ✅ Apply appropriate filter based on page
  if (isHomePage) {
    // HOME: Featured products (showOnHome: true)
    products = products.filter(
      (p) => p.showOnHome === true && p.available === true,
    );
    products = products.slice(0, 16); // Limit to 16 featured products
    products.sort(
      (a, b) => (Number(b.discount) || 0) - (Number(a.discount) || 0),
    ); // Sort by discount
  } else if (isVIPStorePage) {
    // VIP STORE: VIP-only products (isVIPOnly: true)
    products = products.filter(
      (p) =>
        (p.isVIPOnly === true ||
          Number(p.requiredVipLevel || p.requiredVIP || 0) > 0) &&
        p.available === true,
    );
    // Filter by user VIP level
    products = products.filter((p) => {
      const requiredVIP = Number(p.requiredVipLevel || p.requiredVIP || 0);
      return userVipLevel >= requiredVIP; // User must have enough VIP level
    });
  } else if (isMenuPage) {
    // MENU: Regular products (isVIPOnly: false)
    // Hide VIP-only products from regular menu
    products = products.filter(
      (p) =>
        p.isVIPOnly !== true &&
        Number(p.requiredVipLevel || p.requiredVIP || 0) === 0 &&
