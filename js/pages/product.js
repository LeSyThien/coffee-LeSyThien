...
async function renderProductDetail(productId) {
  ...
  try {
    ...
    const vipNotice = isVipLocked
      ? `<div class="badge badge-vip-locked">Bạn cần đạt VIP ${requiredVipLevel} để mua sản phẩm này</div>`
      : requiredVipLevel > 0
      ...
      <div class="product-detail">
        ...
        <div class="product-info">
          ...
          <div class="product-price">
            ${discount > 0 ? `<span class="product-price-strike">${formatCurrency(p.price)}</span><span>${formatCurrency(discountedPrice)}</span>` : formatCurrency(p.price)}
          </div>
          ...
          <div class="product-detail-actions">
            <button
              ...
              ${isVipLocked ? 'data-vip-locked="true"' : ""}
            >
              ${buttonLabel}
            </button>
            <button id="go-back" class="btn-primary btn-secondary-dark" type="button">Back to Shop</button>
          </div>
